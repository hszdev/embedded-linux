...
validate_input() {
    ...
    if [ "$#" -ge 2 ] && ! [[ "$2" =~ ^[0-9]+$ ]]; then
        echo "Error: Amount of photos must be a positive integer"
        usage
        exit 1
    fi    
    if [ "$#" -ge 3 ] && ! [[ "$3" =~ ^[0-9]+$ ]]; then
...
main() {
    ...
    # Capture photos approximately every second for 60 seconds
    local old_photo=""
    for ((i = 0; i < "$amount_of_photos"; i++)); do

        local new_photo=$("$take_photo_script_location" Time "$working_directory")

        # If old photo is empty, then assign to new photo and continue
        if [ -z "$old_photo" ]; then
            old_photo="$new_photo"
